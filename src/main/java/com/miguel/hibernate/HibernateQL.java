...
import com.miguel.hibernate.dominio.ClienteDto;
...
public class HibernateQL {
  public static void main(String[] args) {
    ...
    // Arrays.asList(nombreCliente).forEach(System.out::println);
    ...
    List<ClienteDto> clientesDto = em
        .createQuery("select new com.miguel.hibernate.dominio.ClienteDto(c.nombre, c.apellido) from Cliente c",
            ClienteDto.class)
        .getResultList();
    ...
    // nombres = em.createQuery("select concat(c.nombre, ' ', c.apellido) as
    // nombreCompleto from Cliente c", String.class)
    // .getResultList();
    ...
    // clientes = em.createQuery("select c from Cliente c where c.id between 2 and
    // 5", Cliente.class).getResultList();
    clientes = em.createQuery("select c from Cliente c where c.nombre between 'D' and 'Q'", Cliente.class)
        .getResultList();
    clientes.forEach(System.out::println);
    ...
    registros = em
        .createQuery("select c.nombre, length(c.nombre) from Cliente c order by length(c.nombre) asc", Object[].class)
        .getResultList();
    registros.forEach(reg -> {
    ...
    Object[] estadisticas = em
        .createQuery("select min(c.id), max(c.id), sum(c.id), count(c.id), avg(length(c.nombre)) from Cliente c",
            Object[].class)
        .getSingleResult();
    ...
    Cliente ultimoCliente = em
        .createQuery("select c from Cliente c where c.id = (select max(c.id) from Cliente c)", Cliente.class)
        .getSingleResult();
